...
function Header()
{
  $KsvmUrlImg = '../Vistas/assets/img/medicamentos.png';
    ...
    $this->Cell(80);
    // Título
    $this->Cell(30,10,utf8_decode('Lista de Menús'),0,0,'C');
    // Salto de línea
    $this->Ln(15);
    // Arial bold 11
    ...
    $this->Cell(60,10,utf8_decode('Nivel'),1,0,'C');
    $this->Cell(30,10,utf8_decode('ïcono'),1,0,'C');
    $this->Cell(50,10,utf8_decode('Url'),1,0,'C');
...
$KsvmQuery = $KsvmIniMenu->__KsvmImprimirMenuControlador();
if ($KsvmQuery->rowCount() >= 1) {
    $KsvmListaMenu = $KsvmQuery->fetchAll();
    // Creación del objeto de la clase heredada
$pdf = new PDF();
$pdf->AliasNbPages();
$pdf->AddPage();
$pdf->SetFont('Times','',8);
foreach ($KsvmListaMenu as $row){
    $pdf->Cell(30,8,$row['MnuNomMen'],1,0);
    $pdf->Cell(60,8,$row['MnuNivelMen'],1,0);
    $pdf->Cell(30,8,$row['MnuIconMen'],1,0);
    $pdf->Cell(50,8,$row['MnuUrlMen'],1,0);
    $pdf->Cell(20,8,$row['MnuEstMen'],1,1);
  }
  $pdf->Output();
}
else{
