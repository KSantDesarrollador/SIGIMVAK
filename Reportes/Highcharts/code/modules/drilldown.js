...
(function(e) {
	...
	function n(h, e, n, q) {
		h.hasOwnProperty(e) || (h[e] = q.apply(null, n));
	}
	e = e ? e._modules : {};
	n(
		e,
		...
		function(h, e) {
			...
			var B = h.Tick,
				v = h.fireEvent,
				C = 1;
			q(e.lang, { drillUpText: '\u25c1 Volver a {series.name}' });
			e.drilldown = {
			...
			p.prototype.addSingleSeriesAsDrilldown = function(a, b) {
				...
				var m = this.styledMode
					? { colorIndex: u(a.colorIndex, c.colorIndex) }
					: { color: a.color || c.color };
				this.drilldownLevels || (this.drilldownLevels = []);
				var e = c.options._levelNumber || 0;
				(k = this.drilldownLevels[this.drilldownLevels.length - 1]) &&
					k.levelNumber !== e &&
					(k = void 0);
				b = q(q({ _ddSeriesId: C++ }, m), b);
				var n = c.points.indexOf(a);
				c.chart.series.forEach(function(a) {
					a.xAxis !== d ||
						a.isDrilling ||
						((a.options._ddSeriesId = a.options._ddSeriesId || C++),
						(a.options._colorIndex = a.userOptions._colorIndex),
						(a.options._levelNumber = a.options._levelNumber || e),
						k
							? ((g = k.levelSeries), (l = k.levelSeriesOptions))
							: (g.push(a),
							  (a.purgedOptions = h.merge(
									{
				...
				a = q(
					{
						...
						color: a.isNull ? new h.Color(z).setOpacity(0).get() : z,
				...
				b.options._levelNumber = e + 1;
				d &&
					((d.oldPos = d.pos),
					(d.userMin = d.userMax = null),
					(f.userMin = f.userMax = null));
				c.type === b.type &&
					((b.animate = b.animateDrilldown || y), (b.options.animation = !0));
			};
			...
			p.prototype.getDrilldownBackText = function() {
				...
				if (a && 0 < a.length)
					return (
						...
						E(this.options.lang.drillUpText, a)
					);
			};
			p.prototype.showDrillUpButton = function() {
				...
				if (this.drillUpButton) this.drillUpButton.attr({ text: b }).align();
					...
					this.drillUpButton = this.renderer
						...
						.attr({ align: c.position.align, zIndex: 7 })
						.add()
					...
					for (
						...
						if (((l = b[d]), l.levelNumber === c)) {
							...
							if (!k.chart)
								for (g = f.length; g--; )
									if (
										f[g].options.id === l.lowerSeriesOptions.id &&
										f[g].options._levelNumber === c + 1
									) {
										k = f[g];
										break;
									}
							k.xData = [];
							l.levelSeriesOptions.forEach(e);
							v(a, 'drillup', { seriesOptions: l.seriesOptions });
							m.type === k.type &&
								((m.drilldownLevel = l),
							...
							m.xAxis &&
								((g = l.oldExtremes),
								m.xAxis.setExtremes(g.xMin, g.xMax, !1),
								m.yAxis.setExtremes(g.yMin, g.yMax, !1));
							l.resetZoomButton &&
								((a.resetZoomButton = l.resetZoomButton),
			...
			p.prototype.callbacks.push(function() {
				...
				a.drilldown = {
					update: function(b, c) {
						h.merge(!0, a.options.drilldown, b);
						u(c, !0) && a.redraw();
					}
				};
			});
			r(p, 'beforeShowResetZoom', function() {
				...
				(this.xAxis || []).forEach(function(a) {
					...
					a.series.forEach(function(b) {
						...
						for (c = 0; c < d.length; c++) {
							var g = b.options.data[c];
							'number' !== typeof g &&
								((g = b.pointClass.prototype.optionsToObject.call(
									{ series: b },
									g
								)),
								g.drilldown &&
									(a.ddPoints[d[c]] || (a.ddPoints[d[c]] = []),
					...
					D(function() {
						b.points &&
							b.points.forEach(function(a, b) {
								...
								f &&
									!f.hidden &&
									(f.fadeIn(), a.connector && a.connector.fadeIn());
							});
			...
			t.prototype.animateDrilldown = function(a) {
				var b = this,
					...
					d = c.drilldownLevels,
					f,
					g = n(c.options.drilldown.animation),
					e = this.xAxis,
					k = c.styledMode;
				a ||
					(d.forEach(function(a) {
						b.options._ddSeriesId === a.lowerSeriesOptions._ddSeriesId &&
							((f = a.shapeArgs), k || (f.fill = a.color));
					}),
					(f.x += u(e.oldPos, e.pos) - e.pos),
					this.points.forEach(function(a) {
						var c = a.shapeArgs;
						k || (c.fill = a.color);
						a.graphic &&
							a.graphic
								.attr(f)
								.animate(q(a.shapeArgs, { fill: a.color || b.color }), g);
			...
			t.prototype.animateDrillupFrom = function(a) {
				...
				this.points.forEach(function(g) {
					...
					e &&
						(delete g.graphic,
						f.chart.styledMode || (k.fill = a.color),
						b.duration
							? e.animate(k, h.merge(b, { complete: m }))
							: (e.attr(k), m()));
				});
			};
			A &&
				q(A.prototype, {
					...
					animateDrilldown: function(a) {
						var b = this.chart.drilldownLevels[
						...
						a ||
							(this.points.forEach(function(a, g) {
								var k = a.shapeArgs;
								l || ((d.fill = b.color), (k.fill = a.color));
								if (a.graphic)
									a.graphic
										.attr(
											h.merge(d, { start: f + g * e, end: f + (g + 1) * e })
										)
										[c ? 'animate' : 'attr'](k, c);
							}),
							(this.animate = null));
				...
				v(
					...
					function(b) {
						...
						c &&
							d &&
							(a
								? c.addSingleSeriesAsDrilldown(b.point, d)
								: c.addSeriesAsDrilldown(b.point, d));
					}
				);
			};
			h.Axis.prototype.drilldownCategory = function(a, b) {
				x(this.getDDPoints(a), function(c) {
					c &&
						c.series &&
						c.series.visible &&
						c.doDrilldown &&
			...
			h.Axis.prototype.getDDPoints = function(a) {
			...
			B.prototype.drillable = function() {
				var a = this.pos,
					...
					f = d && c.getDDPoints(a),
					e = c.chart.styledMode;
				d &&
					(b && f && f.length
						? ((b.drillable = !0),
						  b.basicStyles || e || (b.basicStyles = h.merge(b.styles)),
						  b.addClass('highcharts-drilldown-axis-label'),
						  (b.removeOnDrillableClick = r(b.element, 'click', function(b) {
								c.drilldownCategory(a, b);
						  })),
						  e || b.css(c.chart.options.drilldown.activeAxisLabelStyle))
						: b &&
			...
			r(h.Point, 'afterInit', function() {
				var a = this,
					b = a.series;
				a.drilldown &&
					r(a, 'click', function(c) {
						b.xAxis && !1 === b.chart.options.drilldown.allowPointDrilldown
							? b.xAxis.drilldownCategory(a.x, c)
							: a.doDrilldown(void 0, void 0, c);
					});
				return a;
			});
			r(h.Series, 'afterDrawDataLabels', function() {
				var a = this.chart.options.drilldown.activeDataLabelStyle,
					b = this.chart.renderer,
					c = this.chart.styledMode;
				this.points.forEach(function(d) {
					var e = d.options.dataLabels,
						g = u(d.dlOptions, e && e.style, {});
					d.drilldown &&
						d.dataLabel &&
						('contrast' !== a.color ||
							c ||
							(g.color = b.getContrast(d.color || this.color)),
						e && e.color && (g.color = e.color),
						d.dataLabel.addClass('highcharts-drilldown-data-label'),
						c || d.dataLabel.css(a).css(g));
				}, this);
			});
