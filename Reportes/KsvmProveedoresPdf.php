...
function Header()
{
  $KsvmUrlImg = '../Vistas/assets/img/Logo.png';
    ...
    $this->Cell(80);
    // Título
    $this->Cell(30,10,'Lista de Proveedores',0,0,'C');
    // Salto de línea
    $this->Ln(15);
    // Arial bold 11
    $this->SetFont('Arial','B',10);

    $this->Cell(10,10,utf8_decode('Tipo'),1,0,'C');
    $this->Cell(25,10,utf8_decode('# Identidad'),1,0,'C');
    $this->Cell(40,10,utf8_decode('Raz.Social'),1,0,'C');
    $this->Cell(15,10,utf8_decode('Telf'),1,0,'C');
    $this->Cell(30,10,utf8_decode('Email'),1,0,'C');
    $this->Cell(45,10,utf8_decode('Dirección'),1,0,'C');
    $this->Cell(25,10,utf8_decode('Per.Contacto'),1,1,'C');
}
...
$KsvmQuery = $KsvmIniProveedor->__KsvmImprimirProveedorControlador();
if ($KsvmQuery->rowCount() >= 1) {
    $KsvmListaProveedor = $KsvmQuery->fetchAll();
...
$pdf->AddPage();
$pdf->SetFont('Times','',7);
foreach ($KsvmListaProveedor as $row){
    $pdf->Cell(10,8,$row['PvdTipProv'],1,0);
    $pdf->Cell(25,8,$row['PvdIdentProv'],1,0);
    $pdf->Cell(40,8,$row['PvdRazSocProv'],1,0);
    $pdf->Cell(15,8,$row['PvdTelfProv'],1,0);
    $pdf->Cell(30,8,$row['PvdEmailProv'],1,0);
    $pdf->Cell(45,8,$row['PvdDirProv'],1,0);
    $pdf->Cell(25,8,$row['PvdPerContProv'],1,1);
  }
  $pdf->Output();
}
else{
