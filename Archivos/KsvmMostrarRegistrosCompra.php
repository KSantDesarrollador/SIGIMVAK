...
class KsvmMostrarRegistrosCompra extends KsvmEstMaestra
{

    public function __KsvmMostrarRegistros()
    {

    $KsvmMuestraRegistro = "SELECT * FROM ksvmvistaregistroscompra WHERE DocEstOcp = 'N'";
            ...
            $KsvmTable = '<table
                ...
                <thead>
                    <tr>
                        ...
                        <th class="mdl-data-table__cell--non-numeric">V.Unitario
                        </th>
                        <th class="mdl-data-table__cell--non-numeric">Observación
                        </th>
            ...
            $KsvmQuery = $KsvmConsulta->query($KsvmMuestraRegistro);
                ...
                if ($KsvmQuery != null) {
                    ...
                    $KsvmTable .= '
                        ...
                        <td class="mdl-data-table__cell--non-numeric">'.$rows['DocCantOcp'].'</td>
                        <td class="mdl-data-table__cell--non-numeric">'.$rows['DocValorUntOcp'].'</td>
                        <td class="mdl-data-table__cell--non-numeric">'.$rows['DocObservOcp'].'</td>
                        <td style="text-align:center; witdh:30px;">
                        <input type="hidden" name="KsvmCodX" value="'.KsvmEstMaestra::__KsvmEncriptacion($rows['DocId']).'" id="KsvmCodX">  
...
<script type="text/javascript">
   $(document).ready(function(){
    ...
    function KsvmCargarData() {
        $.ajax({
		type: 'POST',
        url: '<?php echo KsvmServUrl;?>Ajax/KsvmMostrarRegistrosCompraAjax.php'
        })
