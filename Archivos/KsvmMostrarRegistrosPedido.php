...
class KsvmMostrarRegistrosPedido extends KsvmEstMaestra
{

    public function __KsvmMostrarRegistros()
    {

    $KsvmMuestraRegistro = "SELECT * FROM ksvmvistaregistrospedido WHERE DrqEstReq = 'N'";
            ...
            $KsvmQuery = $KsvmConsulta->query($KsvmMuestraRegistro);
                ...
                if ($KsvmQuery != null) {
                    ...
                    $KsvmTable .= '
                        ...
                        <td class="mdl-data-table__cell--non-numeric">'.$rows['DrqStockReq'].'</td>
                        <td class="mdl-data-table__cell--non-numeric">'.$rows['DrqCantReq'].'</td>
                        <td class="mdl-data-table__cell--non-numeric">'.$rows['DrqObservReq'].'</td>
                        <td style="text-align:center; witdh:30px;">
                        <input type="hidden" name="KsvmCodX" value="'.KsvmEstMaestra::__KsvmEncriptacion($rows['DrqId']).'" id="KsvmCodX">  
...
<script type="text/javascript">
   $(document).ready(function(){
    ...
    function KsvmCargarData() {
        $.ajax({
		type: 'POST',
        url: '<?php echo KsvmServUrl;?>Ajax/KsvmMostrarRegistrosPedidoAjax.php'
        })
